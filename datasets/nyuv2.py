...
def getTrainingTestingData(path, batch_size, train_tran=getDefaultTrainTransform(), test_tran=getNoTransform()):
    data, nyu2_train = loadZipToMem(path)

    transformed_training = depthDatasetMemory(
        data, nyu2_train, transform=train_tran
    )
    transformed_testing = depthDatasetMemory(
        data, nyu2_train, transform=test_tran
    )
